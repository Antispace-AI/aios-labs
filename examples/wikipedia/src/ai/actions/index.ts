...
export async function wiki_get_article_sections(params) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const sections = fallbackData.parse.sections.map(section => {
        try {
          return {
            index: section.index,
            level: section.level,
  ...
  try {
    ...
    return {
      ...
      results: {
        ...
        url: data.tfa.content_urls?.desktop?.page || `https://en.wikipedia.org/wiki/${encodeURIComponent(data.tfa.title.replace(/ /g, '_'))}`
      }
    };
  } catch (error) {
